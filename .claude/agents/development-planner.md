...
- 수락 기준과 완료 조건 정의
- **API 연동 및 비즈니스 로직 구현 시 반드시 아래 테스트 프로세스를 따름**

**[필수] 테스트 프로세스 (구현 → 테스트 → 검증 → 다음 단계):**

1. **구현**: 작업 파일 명세서에 따라 기능 코드 작성
2. **개발 서버 실행**: `pnpm dev` 실행 후 로컬 환경 준비
3. **Playwright MCP 테스트 실행**:
   - `browser_navigate` → 해당 기능 URL 접근
   - `browser_snapshot` → 페이지 구조 및 접근성 트리 확인
   - `browser_network_requests` → API 요청/응답 상태 코드 및 데이터 검증
   - `browser_click` / `browser_type` → 사용자 인터랙션 시뮬레이션
   - `browser_console_messages({level: 'error'})` → JavaScript 에러 확인
4. **검증**: 모든 테스트 시나리오 통과 확인 후 작업 파일 진행 상황 업데이트
5. **에러 발생 시**: 에러 내용 분석 → 코드 수정 → 2단계부터 재실행
6. **다음 단계 진행**: 전체 테스트 통과 확인 후에만 다음 Task로 이동
...
- **Task 005: 데이터베이스 및 API 개발** - 우선순위
  ...
  - ## 테스트 체크리스트
    - ### 정상 케이스 (Happy Path)
    - [ ] 목록 데이터 로딩: `browser_navigate({url: 'http://localhost:3000'})` → `browser_snapshot()` → 실제 DB 데이터 렌더링 확인
    - [ ] API 응답 검증: `browser_network_requests({includeStatic: false})` → 모든 API 호출 200 응답 확인
    - [ ] 데이터 상세 조회: `browser_click({ref: '항목'})` → `browser_snapshot()` → 상세 데이터 올바르게 표시 확인
    - ### 에러 케이스 (Edge Case)
    - [ ] 잘못된 ID 접근: `browser_navigate({url: '잘못된 리소스 URL'})` → `browser_snapshot()` → 에러 UI 또는 not-found 페이지 표시 확인
    - [ ] API 오류 처리: 네트워크 오류 시 `browser_snapshot()` → 에러 메시지 UI 표시 확인
    - ### 콘솔 오류 확인
    - [ ] `browser_console_messages({level: 'error'})` 실행 후 JavaScript 에러 없음 확인
...
- **Task 006: 인증 및 권한 시스템 구현**
  ...
  - ## 테스트 체크리스트
    - ### 정상 케이스 (Happy Path)
    - [ ] 로그인 성공: `browser_navigate({url: '/login'})` → `browser_type` 자격증명 입력 → `browser_click` 로그인 → `browser_snapshot()` → 인증 후 리다이렉트 확인
    - [ ] 로그아웃: `browser_click({ref: '로그아웃 버튼'})` → `browser_snapshot()` → 로그인 페이지로 이동 확인
    - ### 에러 케이스 (Edge Case)
    - [ ] 미인증 접근 차단: `browser_navigate({url: '/protected'})` → `browser_snapshot()` → 로그인 페이지 리다이렉트 확인
    - [ ] 잘못된 자격증명: `browser_type` 잘못된 비밀번호 입력 → `browser_snapshot()` → 에러 메시지 표시 확인
    - ### 콘솔 오류 확인
    - [ ] `browser_console_messages({level: 'error'})` 실행 후 JavaScript 에러 없음 확인
...
- **Task 006-1: 핵심 기능 통합 테스트**
  - ## 테스트 체크리스트
    - ### 정상 케이스 (Happy Path)
    - [ ] 전체 사용자 플로우: 로그인 → 목록 조회 → 상세 보기 → 액션 수행 → 결과 확인 (각 단계 `browser_snapshot()`)
    - [ ] API 연동 검증: `browser_network_requests({includeStatic: false})` → 모든 API 호출 정상 응답 확인
    - ### 에러 케이스 (Edge Case)
    - [ ] 에러 핸들링 검증: 각 에러 상황에서 사용자에게 적절한 피드백 표시 확인
    - [ ] 엣지 케이스: 빈 데이터, 최대 길이 입력 등 경계값 테스트
    - ### 콘솔 오류 확인
...
**테스트 체크리스트 존재 여부:**
- [ ] API 연동 및 비즈니스 로직 구현 Task에 "## 테스트 체크리스트" 섹션이 포함되었는가?
- [ ] 각 테스트 항목에 사용할 Playwright MCP 도구(`browser_navigate`, `browser_snapshot`, `browser_network_requests`, `browser_click`, `browser_console_messages` 등)가 구체적으로 명시되었는가?
- [ ] Phase 3에 전체 사용자 플로우를 커버하는 통합 테스트 Task가 별도로 포함되었는가?

**테스트 시나리오 완성도:**
- [ ] 정상 케이스(Happy Path): 사용자가 기대하는 모든 주요 플로우가 테스트 항목으로 정의되었는가?
- [ ] 에러 케이스(Edge Case): 잘못된 입력, 존재하지 않는 리소스 접근, API 오류 상황에 대한 테스트 항목이 정의되었는가?
- [ ] 콘솔 오류 확인: `browser_console_messages` 도구를 활용한 JavaScript 에러 점검 항목이 포함되었는가?

**테스트 흐름 명확성:**
- [ ] 각 테스트 항목이 "시나리오 설명 → Playwright MCP 도구 호출 → 기대 결과" 형식으로 작성되었는가?
- [ ] 테스트 실패 시 처리 방법(에러 분석 → 코드 수정 → 재테스트)이 작업 구현 단계에 명시되었는가?
- [ ] 모든 테스트 통과 후에만 다음 Task로 진행하도록 명시되었는가?
